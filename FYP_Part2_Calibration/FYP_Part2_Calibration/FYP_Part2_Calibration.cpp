...
StereoCalib(const char* imageList, int nx, int ny, int useUncalibrated, float _squareSize)
{
	...
	CvSize newSizemx = { 0, 0 };
	CvSize newSizemy = { 0, 0 };
	...
	for (i = 0;; i++)
	{
		...
		{
			printf("%s\n", buf);
			IplImage* cimg = cvCreateImage(imageSize, 8, 3);//8,3
			...
			if (img1 && img2)
			{
				...
				if (!isVerticalStereo)
				{
					...
					for (j = 0; j < imageSize.height; j += 16)
						...
						CV_RGB(0, 255, 0));//lines draw gren color
				}
				else
				{
...
int main(int argc, char *argv[])
{
	...
	StereoCalib(argv[1], nxh, nyv, 0, squareSizec);
	return 0;
}
